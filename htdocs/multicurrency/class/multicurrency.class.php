...
class MultiCurrency extends CommonObject
{
    ...
    */
    public static function getIdAndTxFromCode(&$db, $code, $date_document = '')
    {
        ...
        $sql2= '';
        if (!empty($conf->global->MULTICURRENCY_USE_RATE_ON_DOCUMENT_DATE) && !empty($date_document)) {
            $sql2.= ' AND DATE_FORMAT(mc.date_sync, "%Y-%m-%d") = "'.date('Y-m-d', $date_document).'"';
        }
    ...
    public static function getAmountConversionFromInvoiceRate($fk_facture, $amount, $way = 'dolibarr', $table = 'facture')
    {
        ...
        if ($multicurrency_tx) {
            if ($way == 'dolibarr') {
                return $amount * $multicurrency_tx;
            } else {
                return $amount / $multicurrency_tx;
            }
        } else {
