...
if ($id > 0 && ! preg_match('/^add/i', $action)) {
    ...
    print '</td><td>';
    if ($action == 'edit') {
        print '<input class="flat minwidth200" name="title" value="'.(isset($_POST["title"])?GETPOST("title", '', 2):$object->title).'">';
    } else {
        print $object->title;
    }
    print '</td></tr>';
    ...
    print '</td><td>';
    if ($action == 'edit') {
        print '<input class="flat" name="url" size="80" value="'.(isset($_POST["url"])?$_POST["url"]:$object->url).'">';
    } else {
