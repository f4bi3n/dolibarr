...
if ($resql) {
    ...
    if (!empty($arrayfields['aa.labelshort']['checked'])) {
        print '<td class="liste_titre"><input type="text" class="flat" size="20" name="search_labelshort" value="' . $search_labelshort . '"></td>';
    }
    ...
    if (!empty($arrayfields['aa.pcg_subtype']['checked'])) {
        print_liste_field_titre($arrayfields['aa.pcg_subtype']['label'], $_SERVER["PHP_SELF"], 'aa.pcg_subtype', '', $param, '', $sortfield, $sortorder, '', $arrayfields['aa.pcg_subtype']['help']);
    }
    if (!empty($arrayfields['aa.active']['checked'])) {
