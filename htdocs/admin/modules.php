                            ...
                            try {
                                ...
                                if ($modulequalified > 0) {
                                    ...
                                    if (isset($categ[$specialstring])) {
                                        $categ[$specialstring]++;
                                    } // Array of all different modules categories
                                    else {
...
// Start to show page
if ($mode == 'common') {
    print '<span class="opacitymedium hideonsmartphone">'.$langs->trans("ModulesDesc")."</span><br>\n";
}
if ($mode == 'marketplace') {
    print '<span class="opacitymedium hideonsmartphone">'.$langs->trans("ModulesMarketPlaceDesc")."</span><br>\n";
}
if ($mode == 'deploy') {
    print '<span class="opacitymedium hideonsmartphone">'.$langs->trans("ModulesDeployDesc", $langs->transnoentitiesnoconv("AvailableModules"))."</span><br>\n";
}
if ($mode == 'develop') {
    print '<span class="opacitymedium hideonsmartphone">'.$langs->trans("ModulesDevelopDesc")."</span><br>\n";
}
...
if ($mode == 'common') {
    ...
    foreach ($orders as $key => $value) {
        ...
        if ($mode == 'expdev' && $objMod->version != 'development' && $objMod->version != 'experimental') {
            continue;
        } // Discard if not for current tab
        ...
        if (!empty($objMod->picto)) {
            if (preg_match('/^\//i', $objMod->picto)) {
                print img_picto($alttext, $objMod->picto, 'class="valignmiddle pictomodule"', 1);
            } else {
                print img_object($alttext, $objMod->picto, 'class="valignmiddle pictomodule"');
            }
        } else {
        ...
        if (!empty($conf->global->$const_name)) {	// If module is already activated
            ...
            if (!empty($objMod->config_page_url) && !$disableSetup) {
                $backtourlparam = '';
                if ($search_keyword != '') {
                    $backtourlparam .= ($backtourlparam ? '&' : '?').'search_keyword='.$search_keyword;
                } // No urlencode here, done later
                if ($search_nature > -1) {
