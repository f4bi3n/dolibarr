...
foreach ($rangesbycateg as $fk_c_exp_tax_cat => $Tab) {
    ...
    foreach ($Tab['ranges'] as $k => $range) {
        ...
        if ($action == 'edit' && $range->ik->id == $id && $range->rowid == $fk_range && $range->fk_c_exp_tax_cat == $fk_c_exp_tax_cat) {
            echo '<input type="text" name="offset" value="'.$range->ik->offset.'" />';
        } else {
            echo $range->ik->offset;
        }
        echo '</td>';
        // Coef
        echo '<td width="20%">';
        if ($action == 'edit' && $range->ik->id == $id && $range->rowid == $fk_range && $range->fk_c_exp_tax_cat == $fk_c_exp_tax_cat) {
            echo '<input type="text" name="coef" value="'.$range->ik->coef.'" />';
        } else {
        ...
        if ($range->range_active == 1) {
            if ($action == 'edit' && $range->ik->id == $id && $range->rowid == $fk_range && $range->fk_c_exp_tax_cat == $fk_c_exp_tax_cat) {
                ...
                if (!empty($range->ik->id)) {
                    echo '<a href="'.$_SERVER['PHP_SELF'].'?action=delete&id='.$range->ik->id.'">'.img_delete().'</a>';
                }
