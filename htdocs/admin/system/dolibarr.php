...
if (preg_match('/[a-z]+/i', $version)) {
    $version = 'develop';
} // If version contains text, it is not an official tagged version, so we use the full change log.
    ...
    if (empty($ignore)) {
        ...
        if (preg_match('/^\?/', $key) && empty(${$newkey})) {
            if ($newkey != 'multicompany_transverse_mode' || empty($conf->multicompany->enabled)) {
                continue;
            } // We discard parameters starting with ?
        }
...
if (empty($conf->multicompany->enabled) || !$user->entity) {
    print '<td class="center width="80px"">'.$langs->trans("Entity").'</td>';
