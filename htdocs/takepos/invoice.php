...
if ($action == 'valid' && $user->rights->facture->creer) {
    if ($pay == "cash") {
        $bankaccount = $conf->global->{'CASHDESK_ID_BANKACCOUNT_CASH'.$_SESSION["takeposterminal"]};
    }            // For backward compatibility
    elseif ($pay == "card") {
        $bankaccount = $conf->global->{'CASHDESK_ID_BANKACCOUNT_CB'.$_SESSION["takeposterminal"]};
    }          // For backward compatibility
    elseif ($pay == "cheque") {
        $bankaccount = $conf->global->{'CASHDESK_ID_BANKACCOUNT_CHEQUE'.$_SESSION["takeposterminal"]};
    }    // For backward compatibility
    else {
    ...
    if (! empty($conf->global->TAKEPOS_SUPPLEMENTS)) {
        ...
        if ($found !== false) { // If this product is a supplement
            ...
            if ($row[0]==null) {
                ...
                $parent_line=$row[0];
            } //If the parent line is already a supplement, add the supplement to the main  product
        }
    }
    ...
    if (!empty($conf->adherent->enabled)) {
        ...
        if ($result > 0) {
            ...
            if ($adh->datefin) {
                ...
                if ($adh->statut > 0) {
                    print " ".img_warning($langs->trans("Late"));
                } // displays delay Pictogram only if not a draft and not terminated
            }
        } else {
