    ...
    */
    ...
    public function __construct($db)
    {
        ...
        if (!empty($conf->global->SOCIETE_DISABLE_CONTACTS)) {
            $this->fields['thirdparty']['enabled'] = 0;
        }
    ...
    public function getNomUrl($withpicto = 0, $option = '', $maxlen = 0, $moreparam = '', $save_lastsearch_value = -1, $notooltip = 0)
    {
        ...
        $phonelist = array();
        if ($this->phone_pro) {
            $phonelist[] = dol_print_phone($this->phone_pro, $this->country_code, $this->id, 0, '', '&nbsp;', 'phone');
        }
        if ($this->phone_mobile) {
            $phonelist[] = dol_print_phone($this->phone_mobile, $this->country_code, $this->id, 0, '', '&nbsp;', 'mobile');
        }
        if ($this->phone_perso) {
            $phonelist[] = dol_print_phone($this->phone_perso, $this->country_code, $this->id, 0, '', '&nbsp;', 'phone');
        }
