...
if ($action == 'renamefile') {
    $action = 'file_manager';
} // After actions_linkedfiles, if action were renamefile, we set it to 'file_manager'
            ...
            if ($tmp['curl_error_no']) {
                ...
                if (preg_match('/<meta name="title"[^"]+content="([^"]+)"/ims', $head, $regtmp)) {
                    if (empty($objectpage->title)) {
                        $objectpage->title = $regtmp[1];
                    } // If title not found into <title>, we get it from <meta title>
                }
            ...
            if (is_array($tmpaltaliases)) {
                foreach ($tmpaltaliases as $tmpaliasalt) {
                    if (trim($tmpaliasalt)) {
                        ...
                        if (!$result) {
                            setEventMessages('Failed to write file '.$pathofwebsite.'/'.trim($tmpaliasalt).'.php', null, 'errors');
                        }
                    }
                }
            }
...
$style = '';
if ($action != 'preview' && $action != 'editcontent' && $action != 'editsource') {
...
if (!GETPOST('hide_websitemenu')) {
    ...
    if (in_array($action, array('editcss', 'editmenu', 'file_manager', 'replacesite', 'replacesiteconfirm'))) {
        ...
        if (preg_match('/^create/', $action) && $action != 'file_manager' && $action != 'replacesite' && $action != 'replacesiteconfirm') {
            print '<input type="submit" id="savefile" class="button buttonforacesave" value="'.dol_escape_htmltag($langs->trans("Save")).'" name="update">';
        }
        if (preg_match('/^edit/', $action) && $action != 'file_manager' && $action != 'replacesite' && $action != 'replacesiteconfirm') {
            print '<input type="submit" id="savefile" class="button buttonforacesave" value="'.dol_escape_htmltag($langs->trans("Save")).'" name="update">';
        }
        if ($action != 'preview') {
            print '<input type="submit" class="button" value="'.dol_escape_htmltag($langs->trans("Cancel")).'" name="cancel">';
        }
    }
