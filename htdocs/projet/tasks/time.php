    ...
    if (!$error) {
        ...
        if (!$error) {
            ...
            if (empty($object->project->statut)) {
                ...
                } // If progress is -1 (not defined), we do not change value
...
if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0) {
    ...
    if ($projectstatic->id > 0) {
        ...
        if (!empty($arrayfields['author']['checked'])) {
            print '<td class="liste_titre">'.$form->select_dolusers(($search_user > 0 ? $search_user : -1), 'search_user', 1, null, 0, '', '', 0, 0, 0, '', 0, '', 'maxwidth200').'</td>';
        }
        // Note
        ...
        */
        ...
        foreach ($tasks as $task_time) {
            ...
            if (!empty($arrayfields['valuebilled']['checked'])) {
                ...
                if (!$i) {
                    $totalarray['totalvaluebilledfield'] = $totalarray['nbfield'];
                }
