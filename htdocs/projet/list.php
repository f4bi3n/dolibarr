...
} // To avoid sql syntax error if not found
...
if (!empty($search_categ)) {
    $sql .= ' LEFT JOIN '.MAIN_DB_PREFIX."categorie_project as cs ON p.rowid = cs.fk_project";
} // We'll need this table joined to the select in order to filter by categ
...
} // This filter if when we use a hard coded filter on company on url (not related to filter for external users)
if ($search_categ > 0) {
...
if ($search_all != '') {
    $param .= '&search_all='.$search_all;
}
if ($search_sday) {
    $param .= '&search_sday='.$search_sday;
}
if ($search_smonth) {
    $param .= '&search_smonth='.$search_smonth;
}
if ($search_syear) {
    $param .= '&search_syear='.$search_syear;
}
if ($search_eday) {
    $param .= '&search_eday='.$search_eday;
}
if ($search_emonth) {
    $param .= '&search_emonth='.$search_emonth;
}
if ($search_eyear) {
    $param .= '&search_eyear='.$search_eyear;
}
if ($socid) {
    $param .= '&socid='.$socid;
}
...
if ($search_public != '') {
    $param .= '&search_public='.$search_public;
}
if ($search_project_user != '') {
    $param .= '&search_project_user='.$search_project_user;
}
if ($search_sale > 0) {
    $param .= '&search_sale='.$search_sale;
}
if ($search_opp_amount != '') {
    $param .= '&search_opp_amount='.$search_opp_amount;
}
if ($search_budget_amount != '') {
    $param .= '&search_budget_amount='.$search_budget_amount;
}
if ($optioncss != '') {
