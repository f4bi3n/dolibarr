        ...
        if (count($elementarray)>0 && is_array($elementarray)) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                if ($key == 'invoice') {
                    ...
                    }	// Replacement invoice, do not include into total
                    if (! empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS) && $element->type == Facture::TYPE_DEPOSIT) {
                        $qualifiedfortotal=false;
                    }	// If hidden option to use deposits as payment (deprecated, not recommended to use this), deposits are not included
                }
                if ($key == 'propal') {
                    if ($element->statut == Propal::STATUS_NOTSIGNED) {
                        $qualifiedfortotal=false;
                    }	// Refused proposal must not be included in total
                }
...
foreach ($listofreferent as $key => $value) {
    ...
    if ($qualified) {
        ...
        if (empty($conf->global->PROJECT_CREATE_ON_OVERVIEW_DISABLED) && $urlnew) {
            $addform.='<div class="inline-block valignmiddle">';
            if ($testnew) {
                $addform.='<a class="buttonxxx" href="'.$urlnew.'"><span class="valignmiddle text-plus-circle">'.($buttonnew?$langs->trans($buttonnew):$langs->trans("Create")).'</span><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
        ...
        if (in_array($tablename, array('projet_task')) && $key == 'project_task') {
            print '';
        } // if $key == 'project_task', we don't want details per user
        elseif (in_array($tablename, array('payment_various'))) {
            print '';
        } // if $key == 'payment_various', we don't have any thirdparty
        ...
        if (is_array($elementarray) && count($elementarray) > 0) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                elseif ($tablename == 'stock_mouvement') {
                    ...
                    if ($tablename == 'commande_fournisseur' || $tablename == 'supplier_order') {
                        ...
                        $date = $element->date_validation;
                    } // There is no other date for this
                    elseif ($tablename == 'fichinter') {
                        $date = $element->datev;
                    } // There is no other date for this
                    elseif ($tablename == 'projet_task') {
                        $date = '';
                    } // We show no date. Showing date of beginning of task make user think it is date of time consumed
                    else {
            ...
            print '<td class="right">';
            if (empty($value['disableamount'])) {
                if ($tablename != 'projet_task' || !empty($conf->salaries->enabled)) {
