...
if ($billed != '' && $billed >= 0) {
    $sql .= ' AND c.facture = '.$billed;
}
if ($viewstatut <> '') {
    if ($viewstatut < 4 && $viewstatut > -3) {
        if ($viewstatut == 1 && empty($conf->expedition->enabled)) {
            $sql .= ' AND c.fk_statut IN (1,2)';
        } // If module expedition disabled, we include order with status 'sending in process' into 'validated'
        else {
            $sql .= ' AND c.fk_statut = '.$viewstatut;
        } // brouillon, validee, en cours, annulee
    }
    if ($viewstatut == 4) {
...
if ($resql) {
    ...
    if ($search_dateorder_start) {
        $param .= '&search_dateorder_start='.urlencode($search_dateorder_start);
    }
    if ($search_dateorder_end) {
        $param .= '&search_dateorder_end='.urlencode($search_dateorder_end);
    }
    if ($search_datedelivery_start) {
    ...
    if (!empty($arrayfields['s.town']['checked'])) {
        print '<td class="liste_titre"><input class="flat" type="text" size="4" name="search_town" value="'.$search_town.'"></td>';
    }
    // Zip
    ...
    if (!empty($arrayfields['c.total_vat']['checked'])) {
        print_liste_field_titre($arrayfields['c.total_vat']['label'], $_SERVER["PHP_SELF"], 'c.tva', '', $param, '', $sortfield, $sortorder, 'right ');
    }
    if (!empty($arrayfields['c.total_ttc']['checked'])) {
