...
if ($search_date_start) {
    $sql .= " AND v.datep >= '".$db->idate($search_date_start)."'";
}
if ($search_date_end) {
    $sql .= " AND v.datep <= '".$db->idate($search_date_end)."'";
}
...
if ($result) {
    ...
    if ($search_account > 0) {
        $param .= '&search_amount='.urlencode($search_account);
    }
    if ($search_accountancy_account > 0) {
    ...
    if (!empty($conf->accounting->enabled)) {
        print_liste_field_titre("AccountAccountingShort", $_SERVER["PHP_SELF"], "v.accountancy_code", "", $param, '', $sortfield, $sortorder, 'left ');
    }
    if (!empty($conf->accounting->enabled)) {
        print_liste_field_titre("SubledgerAccount", $_SERVER["PHP_SELF"], "v.subledger_account", "", $param, '', $sortfield, $sortorder, 'left ');
    }
