...
class Facture extends CommonInvoice
{
    ...
    public function getNomUrl($withpicto = 0, $option = '', $max = 0, $short = 0, $moretitle = '', $notooltip = 0, $addlinktonotes = 0, $save_lastsearch_value = -1, $target = '')
    {
        ...
        if ($user->rights->facture->lire) {
            ...
            if ($this->type == self::TYPE_REPLACEMENT) {
                $label = '<u>'.$langs->transnoentitiesnoconv("ShowInvoiceReplace").'</u>';
            }
            if ($this->type == self::TYPE_CREDIT_NOTE) {
                $label = '<u>'.$langs->transnoentitiesnoconv("ShowInvoiceAvoir").'</u>';
            }
            if ($this->type == self::TYPE_DEPOSIT) {
                $label = '<u>'.$langs->transnoentitiesnoconv("ShowInvoiceDeposit").'</u>';
            }
            if ($this->type == self::TYPE_SITUATION) {
                $label = '<u>'.$langs->transnoentitiesnoconv("ShowInvoiceSituation").'</u>';
            }
            if (!empty($this->ref)) {
