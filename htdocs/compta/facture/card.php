...
if (empty($reshook)) {
    ...
    elseif ($action == 'valid' && $usercancreate) {
        ...
        if ($object->type == Facture::TYPE_CREDIT_NOTE) {
            ...
            foreach ($object->lines as $line) {
                if (empty($array_of_pu_ht_per_vat_rate[$line->tva_tx.'_'.$line->vat_src_code])) {
                    $array_of_pu_ht_per_vat_rate[$line->tva_tx.'_'.$line->vat_src_code] = 0;
                }
                if (empty($array_of_pu_ht_devise_per_vat_rate[$line->tva_tx.'_'.$line->vat_src_code])) {
                    $array_of_pu_ht_devise_per_vat_rate[$line->tva_tx.'_'.$line->vat_src_code] = 0;
                }
    ...
    elseif ($action == 'confirm_converttoreduc' && $confirm == 'yes' && $usercancreate) {
        ...
        if ($object->type == Facture::TYPE_DEPOSIT && empty($discountcheck->id)) {
            $canconvert = 1;
        } // we can convert deposit into discount if deposit is payed (completely, partially or not at all) and not already converted (see real condition into condition used to show button converttoreduc)
        if (($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_STANDARD) && $object->paye == 0 && empty($discountcheck->id)) {
            $canconvert = 1;
