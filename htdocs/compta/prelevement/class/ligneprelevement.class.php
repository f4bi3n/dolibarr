...
class LignePrelevement
{
    ...
    public function LibStatut($status, $mode = 0)
    {
        ...
        if ($mode == 0) {
            ...
            if ($status==0) {
                return img_picto($langs->trans($this->statuts[$status]), 'statut1').' '.$langs->trans($this->statuts[$status]);
            }   // Waiting
            elseif ($status==2) {
                return img_picto($langs->trans($this->statuts[$status]), 'statut6').' '.$langs->trans($this->statuts[$status]);
            }   // Credited
            elseif ($status==3) {
