...
if (empty($action) || $action == 'list') {
    ...
    if ($search_ref) {
        $sql .= natural_search('p.rowid', $search_ref);
    }
    if ($search_account > 0) {
    ...
    if ($resql) {
        ...
        while ($i < min($num, $limit)) {
            ...
            print '<td>';
            if ($objp->socid) {
                print '<a href="'.DOL_URL_ROOT.'/societe/card.php?socid='.$objp->socid.'">'.img_object($langs->trans('ShowCompany'), 'company').' '.dol_trunc($objp->name, 32).'</a>';
            } else {
                print '&nbsp;';
            }
            print '</td>';
