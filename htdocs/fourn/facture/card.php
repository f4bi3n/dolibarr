...
if (empty($reshook)) {
    ...
    elseif ($action == 'confirm_converttoreduc' && $confirm == 'yes' && $usercancreate) {
        ...
        if (($object->type == FactureFournisseur::TYPE_CREDIT_NOTE || $object->type == FactureFournisseur::TYPE_STANDARD) && $object->paye == 0 && empty($discountcheck->id)) {
            $canconvert = 1;
        } // we can convert credit note into discount if credit note is not refunded completely and not already converted and amount of payment is 0 (see also the real condition used as the condition to show button converttoreduc)
        if ($canconvert) {
            $db->begin();
    ...
    elseif ($action == 'updateline' && $user->rights->fournisseur->facture->creer) {
        ...
        if ($prod_entry_mode != 'free' && empty($error)) {	// With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
            ...
            if ($idprod > 0) {
                ...
                if (empty($pu)) {
                    $pu = 0;
                } // If pu is '' or null, we force to have a numeric value
