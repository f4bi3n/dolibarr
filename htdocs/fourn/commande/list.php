...
if ($status) {
    ...
    if ($status == '6,7') {
        ...
        $title .= ' - '.$commandestatic->LibStatut($status);
    }
}
if ($search_billed > 0) {
...
if ($resql) {
    ...
    if ($sall) {
        $param .= "&search_all=".$sall;
    }
    if ($search_orderday) {
        $param .= '&search_orderday='.$search_orderday;
    }
    if ($search_ordermonth) {
        $param .= '&search_ordermonth='.$search_ordermonth;
    }
    if ($search_orderyear) {
        $param .= '&search_orderyear='.$search_orderyear;
    }
    if ($search_deliveryday) {
        $param .= '&search_deliveryday='.$search_deliveryday;
    }
    if ($search_deliverymonth) {
        $param .= '&search_deliverymonth='.$search_deliverymonth;
    }
    if ($search_deliveryyear) {
        $param .= '&search_deliveryyear='.$search_deliveryyear;
    }
    if ($search_ref) {
    ...
    if ($search_request_author) {
        $param .= '&search_request_author='.$search_request_author;
    }
    if ($search_sale > 0) {
    ...
    if ($search_total_ttc != '') {
        $param .= "&search_total_ttc=".$search_total_ttc;
    }
    if ($search_refsupp) {
    ...
    if ($search_project_ref >= 0) {
        $param .= "&search_project_ref=".$search_project_ref;
    }
    if ($search_billed != '') {
        $param .= "&search_billed=".$search_billed;
    }
    if ($show_files) {
        $param .= '&show_files='.$show_files;
    }
    if ($optioncss != '') {
    ...
    if (!empty($arrayfields['cf.ref_supplier']['checked'])) {
        print_liste_field_titre($arrayfields['cf.ref_supplier']['label'], $_SERVER["PHP_SELF"], "cf.ref_supplier", "", $param, '', $sortfield, $sortorder);
    }
    if (!empty($arrayfields['p.project_ref']['checked'])) {
