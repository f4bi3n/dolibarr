...
<?php } else {
    ...
    if (empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
        ...
        if (price2num($line->total_localtax1)) {
            print '<br>'.$langs->transcountry("TotalLT1", ($senderissupplier ? $object->thirdparty->country_code : $mysoc->country_code)).'='.price($line->total_localtax1);
        }
        if (price2num($line->total_localtax2)) {
            print '<br>'.$langs->transcountry("TotalLT2", ($senderissupplier ? $object->thirdparty->country_code : $mysoc->country_code)).'='.price($line->total_localtax2);
        }
