            ...
            if (!empty($conf->global->AGENDA_USE_EVENT_TYPE)) {
                if ($actionstatic->type_picto) {
                    ...
                    if ($actionstatic->type_code == 'AC_RDV') {
                        $out .= img_picto('', 'object_group', '', false, 0, 0, '', 'paddingright').' ';
                    } elseif ($actionstatic->type_code == 'AC_TEL') {
                        $out .= img_picto('', 'object_phoning', '', false, 0, 0, '', 'paddingright').' ';
                    } elseif ($actionstatic->type_code == 'AC_FAX') {
                        $out .= img_picto('', 'object_phoning_fax', '', false, 0, 0, '', 'paddingright').' ';
                    } elseif ($actionstatic->type_code == 'AC_EMAIL') {
                        $out .= img_picto('', 'object_email', '', false, 0, 0, '', 'paddingright').' ';
                    } elseif ($actionstatic->type_code == 'AC_INT') {
                        $out .= img_picto('', 'object_intervention', '', false, 0, 0, '', 'paddingright').' ';
                    } elseif (!preg_match('/_AUTO/', $actionstatic->type_code)) {
                        $out .= img_picto('', 'object_action', '', false, 0, 0, '', 'paddingright').' ';
                    }
                }
            }
