...
function run_sql($sqlfile, $silent = 1, $entity = '', $usesavepoint = 1, $handler = '', $okerror = 'default', $linelengthlimit = 32768, $nocommentremoval = 0, $offsetforchartofaccount = 0)
{
    ...
    if ($error == 0) {
        ...
        if (!$silent) {
            print '<td class="right">'.$langs->trans("OK").'</td></tr>'."\n";
        }
        $ok = 1;
    } else {
        if (!$silent) {
        ...
        if (!$silent) {
            print '<td class="right"><font class="error">'.$langs->trans("KO").'</font></td></tr>'."\n";
        }
        $ok = 0;
    }
...
function security_prepare_head()
{
    ...
    */
    ...
    if ($nbPerms > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">'.$nbPerms.'</span>';
    }
    $head[$h][2] = 'default';
    $h++;
