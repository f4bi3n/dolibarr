    ...
    public function insertExtraFields($trigger = '', $userused = null)
    {
        ...
        if (!empty($this->array_options)) {
            ...
            if (is_array($extrafields->attributes[$this->table_element]['mandatoryfieldsofotherentities'])) {
                foreach ($extrafields->attributes[$this->table_element]['mandatoryfieldsofotherentities'] as  $tmpkey => $tmpval) {
                    if (!isset($extrafields->attributes[$this->table_element]['type'][$tmpkey])) {    // If field not already added previously
                        if (in_array($tmpval, array('int', 'double'))) {
                            $sql .= ", 0";
                        } else {
                            $sql .= ", ''";
                        }
    ...
    public function showOptionals($extrafields, $mode = 'view', $params = null, $keysuffix = '', $keyprefix = '', $onetrtd = 0)
    {
        ...
        if (is_array($extrafields->attributes[$this->table_element]['label']) && count($extrafields->attributes[$this->table_element]['label']) > 0) {
            ...
            foreach ($extrafields->attributes[$this->table_element]['label'] as $key=>$label) {
                ...
                if (($mode == 'create' || $mode == 'edit') && abs($visibility) != 1 && abs($visibility) != 3) {
                    continue;
                ...
                if ($extrafields->attributes[$this->table_element]['type'][$key] == 'separate') {
                    ...
                    if ($tpl_context == "public") {	// Public page : red dot instead of fieldrequired characters
                        ...
                        if (!empty($extrafields->attributes[$this->table_element]['help'][$key])) {
                            $out .= $form->textwithpicto($labeltoshow, $extrafields->attributes[$this->table_element]['help'][$key]);
                        } else {
                            $out .= $labeltoshow;
                        }
