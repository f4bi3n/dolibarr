...
class Adherent extends CommonObject
{
    ...
    public function update($user, $notrigger = 0, $nosyncuser = 0, $nosyncuserpass = 0, $nosyncthirdparty = 0, $action = 'update')
    {
        ...
        }			// Must be modified only when creating from a third-party
        if ($this->datefin) {
    ...
    public function getNomUrl($withpictoimg = 0, $maxlen = 0, $option = 'card', $mode = '', $morecss = '', $save_lastsearch_value = -1, $notooltip = 0)
    {
        ...
        if ($withpictoimg) {
            ...
            // Only picto
            if ($withpictoimg > 0) {
                $picto='<span class="nopadding'.($morecss?' userimg'.$morecss:'').'">'.img_object('', 'user', $paddafterimage.' '.($notooltip?'':'class="classfortooltip"'), 0, 0, $notooltip?0:1).'</span>';
            }
        ...
        if ($withpictoimg > -2 && $withpictoimg != 2) {
            if (empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
                $result.='<span class="nopadding valignmiddle'.((! isset($this->statut) || $this->statut)?'':' strikefordisabled').($morecss?' usertext'.$morecss:'').'">';
            }
            if ($mode == 'login') {
    ...
    public function _load_ldap_info()
    {
        ...
        if ($this->twitter && ! empty($conf->global->LDAP_MEMBER_FIELD_TWITTER)) {
            $info[$conf->global->LDAP_MEMBER_FIELD_TWITTER] = $this->twitter;
        }
        if ($this->facebook && ! empty($conf->global->LDAP_MEMBER_FIELD_FACEBOOK)) {
            $info[$conf->global->LDAP_MEMBER_FIELD_FACEBOOK] = $this->facebook;
        }
        if ($this->linkedin && ! empty($conf->global->LDAP_MEMBER_FIELD_LINKEDIN)) {
            $info[$conf->global->LDAP_MEMBER_FIELD_LINKEDIN] = $this->linkedin;
        }
