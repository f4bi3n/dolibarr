...
if ($search_gender != '' && $search_gender != '-1') {
    $sql .= " AND d.gender = '".$search_gender."'";
}
if ($search_login) {
...
if ($filter == 'outofdate') {
    $sql .= " AND ((datefin IS NULL OR datefin < '".$db->idate($now)."') AND t.subscription = 1)";
}
...
if ($num > $limit || $page) {
    print_barre_liste('', $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'generic', 0, '', '', $limit, 1);
}
