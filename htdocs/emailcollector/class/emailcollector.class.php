    ...
    */
    ...
    private function overwritePropertiesOfObject(&$object, $actionparam, $messagetext, $subject, $header)
    {
        ...
        foreach ($arrayvaluetouse as $propertytooverwrite => $valueforproperty) {
            ...
            if ($tmpclass && ($tmpclass != $object->element)) {
                continue;
            } // Property is for another type of object
            ...
            if ($tmpproperty) {
                ...
                if (!empty($sourcefield) && !empty($regexstring)) {
                    ...
                    if ($sourcestring) {
                        ...
                        if (strtolower($sourcefield) == 'body') {
                            $regexoptions = 'ms';
                        } // The m means ^ and $ char is valid at each new line. The s means the char '.' is valid for new lines char too
                        if (strtolower($sourcefield) == 'header') {
                            $regexoptions = 'm';
                        } // The m means ^ and $ char is valid at each new line.
    ...
    public function doCollectOneCollector()
    {
        ...
        if (empty($targetdir)) {	// Use last date as filter if there is no targetdir defined.
            ...
            if ($fromdate > 0) {
                $search .= ($search ? ' ' : '').'SINCE '.date('j-M-Y', $fromdate - 1);
                ...
                foreach ($this->actions as $operation) {
                    ...
                    if (in_array($operation['type'], array('recordevent', 'project', 'ticket'))) {
                        if ($operation['type'] == 'project') {
                            $descriptiontitle = $langs->trans("ProjectCreatedByEmailCollector", $msgid);
                        } elseif ($operation['type'] == 'ticket') {
                            $descriptiontitle = $langs->trans("TicketCreatedByEmailCollector", $msgid);
                        } else {
                            $descriptiontitle = $langs->trans("ActionAC_".$actioncode).' - '.$langs->trans("MailFrom").' '.$from;
                        }
